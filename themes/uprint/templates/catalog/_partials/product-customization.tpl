...
<section class="product-customization">
  {if !$configuration.is_catalog}
    <div class="card card-block">
      <p class="h4 card-title">Escoger una opción</p>
            ...
            {foreach from=$customizations.fields item="field"}
              <li class="product-customization-item">
                {if $field.type == 'text'}
                  ...
                  <textarea placeholder="{$field.label}" class="product-message" maxlength="250" {if $field.required} required {/if} name="{$field.input_name}"></textarea>
                ...
                {elseif $field.type == 'image'}
                  ...
                  <span class="custom-file">
                    ...
                    <input id="additionalOptionsFile" class="file-input js-file-input" {if $field.required} required {/if} type="file" name="{$field.input_name}">
                    {* <button class="btn btn-primary">{l s='Choose file' d='Shop.Theme.Actions'}</button> *}
                    <p class="small">En caso de no tener un archivo indicar link para descarga</p>                  
                  </span>
                  {* <small class="float-xs-right">{l s='.PDF' d='Shop.Forms.Help'}</small> *}
                {/if}
              </li>
            {/foreach}
          </ul>
          <div class="clearfix">
            <button class="btn btn-info btn-info" type="submit" name="submitCustomizedData">Guardar información</button>
          </div>
        </form>
      {/block}
