...
class mgmbannersdest extends Module implements WidgetInterface {
    public $controls = array();
    //put your code here
    public function __construct() {
        
        $this->name = "mgmbannersdest";
        ...
        $this->displayName = $this->trans('mgmbannersdest', array(), 'Module.mymodule');
        
        $this->description = $this->trans('Módulo Banners de destacados.', array(), 'Module.mymodule');
    ...
    protected function postProcess() {
        ...
        $MYMODULE_URL_FILE = $this->uploadFiles('MYMODULE_URL_FILE1');
            ...
            Db::getInstance()->insert('mgmbannersdestacados_configuracion', array(
    ...
    protected function uploadFiles($field){
        ...
        foreach($_FILES as $key => $value):
            if($key == $field){
                ...
                $folder = _PS_MODULE_DIR_.'mgmbannersdest/documents/';
                if(strpos($ext, 'php') === false):
                        if($this->uploadFile($folder, $_FILES[$key]['tmp_name'], $name)):
                                $file = _PS_BASE_URL_._MODULE_DIR_.'mgmbannersdest/documents/'.$name;
                        endif;
                endif;   
            }                 
        endforeach;
            ...
            'form' => array(
                ...
                'input' => array( 
                    ...
                    array(
                        ...
                        'display_image' => true,
                        'desc' => $this->l('Introduce Imagen de Banner.'),
                        'label' => $this->l('Imagen de Banner'),
                    ),
                ),
                'submit' => array(
                    'title' => $this->l('Save'),
    ...
    public function setMedia()
    {
        ...
        $this->context->controller->addCSS($this->local_path.'views/css/mgmbannersdest.css');


        $this->context->controller->addJS($this->local_path.'views/js/slick.js');
        $this->context->controller->addJS($this->local_path.'views/js/mgmbannersdest.js');
    ...
    public function getData() {
        $results = (Db::getInstance())->executeS((new DbQuery())
            ->select('*')
            ->from('mgmbannersdestacados_configuracion')
            ->where('1')
            ->orderBy('id DESC')
            ->limit(6)  
        );
    ...
    public function getImageDelete() {
        $sql = 'SELECT * FROM '._DB_PREFIX_. 'mgmbannersdestacados_configuracion WHERE 1 ORDER BY id DESC LIMIT 6';
    ...
    public function delete_data(){
        ...
        Db::getInstance()->delete('mgmbannersdestacados_configuracion', 'id = '.$id, 1);
    }
